...
import { UserPlus, LockKeyhole, Eye, EyeOff, UserRound, Mail } from 'lucide-vue-next'
...
async function register() {
  ...
  loading.value = true
  try {
    const data = await api.register(username.value, password.value, password2.value, email.value)
    localStorage.setItem('shopuz-token', data.token)
    localStorage.setItem('shopuz-username', data.username)
    router.push('/')
  } catch (e) {
    error.value = e.message || 'Ro\'yxatdan o\'tishda xatolik.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="flex min-h-[70vh] items-center justify-center py-[30px]">
    <div class="w-full max-w-[430px] rounded-[22px] border border-white/10 bg-[#0b1613] p-[25px] shadow-2xl">
      ...
      <h1 class="mt-[18px] text-center text-[26px] font-black">Ro'yxatdan o'tish</h1>
      <p class="mt-[6px] text-center text-[12px] text-white/40">Yangi ShopUz akkaunt yarating</p>

      <form @submit.prevent="register" class="mt-[25px] space-y-[13px]">
        <div>
          <label class="mb-[7px] block text-[11px] font-bold text-white/60">Login</label>
          <div
            class="flex items-center rounded-[11px] border border-white/10 bg-[#101b18] focus-within:border-[#19f078]/60">
        ...
        <div>
          <label class="mb-[7px] block text-[11px] font-bold text-white/60">Email (ixtiyoriy)</label>
          <div
            class="flex items-center rounded-[11px] border border-white/10 bg-[#101b18] focus-within:border-[#19f078]/60">
            <Mail :size="17" class="ml-[13px] text-white/35" />
            <input v-model="email" type="email" autocomplete="email"
              class="h-[46px] w-full bg-transparent px-[10px] text-[13px] outline-none"
        ...
        <div>
          <label class="mb-[7px] block text-[11px] font-bold text-white/60">Parolni takrorlang</label>
          <div
            class="flex items-center rounded-[11px] border border-white/10 bg-[#101b18] focus-within:border-[#19f078]/60">
            <LockKeyhole :size="17" class="ml-[13px] text-white/35" />
            <input v-model="password2" :type="show ? 'text' : 'password'" autocomplete="new-password"
              class="h-[46px] w-full bg-transparent px-[10px] text-[13px] outline-none" placeholder="••••••••" />
          </div>
        </div>
        <p v-if="error"
