...
import { Search, ShoppingCart, UserRound, Heart, Menu, X, Home, LayoutGrid, Smartphone, Laptop, Headphones, WashingMachine, Shirt, Dumbbell, Sparkles } from 'lucide-vue-next'
...
<div class="min-h-screen bg-[#020807] text-white">
  <header v-if="!hideNav" class="sticky top-0 z-50 border-b border-white/10 bg-[#06100d]/95 backdrop-blur-xl">
    <div class="mx-auto flex h-[72px] max-w-[1440px] items-center gap-[18px] px-[18px] lg:px-[30px]">
      ...
      <nav class="ml-auto hidden items-center gap-[8px] md:flex"><RouterLink to="/kategoriya/telefon" class="flex items-center gap-[6px] rounded-[10px] px-[13px] py-[10px] text-[13px] text-white/75 hover:bg-white/5 hover:text-white"><LayoutGrid :size="17"/> Kategoriyalar</RouterLink><RouterLink to="/savatcha" class="relative flex items-center gap-[6px] rounded-[10px] px-[13px] py-[10px] text-[13px] text-white/75 hover:bg-white/5"><ShoppingCart :size="17"/> Savatcha <span v-if="count" class="ml-[3px] rounded-full bg-[#19f078] px-[6px] py-[2px] text-[10px] font-bold text-black">{{count}}</span></RouterLink><RouterLink v-if="!username" to="/login" class="flex items-center gap-[6px] rounded-[10px] px-[13px] py-[10px] text-[13px] text-white/75 hover:bg-white/5"><UserRound :size="17"/> Kirish</RouterLink><button v-else @click="logout" class="flex items-center gap-[6px] rounded-[10px] px-[13px] py-[10px] text-[13px] text-white/75 hover:bg-white/5"><UserRound :size="17"/> {{ username }} · Chiqish</button></nav>
    </div>
    ...
    <div v-if="open" class="border-t border-white/10 bg-[#06100d] p-[15px] lg:hidden"><RouterLink v-for="c in cats" :key="c[0]" @click="open=false" :to="`/kategoriya/${c[0]}`" class="flex items-center gap-[10px] rounded-[10px] p-[11px] hover:bg-white/5"><component :is="c[2]" :size="18"/> {{c[1]}}</RouterLink><RouterLink @click="open=false" to="/admin/login" class="mt-[6px] flex items-center gap-[10px] rounded-[10px] border border-[#19f078]/20 p-[11px] font-bold text-[#19f078] hover:bg-[#19f078]/10">Admin panel</RouterLink></div>
  </header>
